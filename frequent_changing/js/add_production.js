$(function() {
    ...
    $(document).on('change', '#ingredient_id', function() {
        ...
        if (ingredient_details != '') {
            ...
            let cart_row = '<tr class="rowCount" data-item_id="' + ingredient_details_array[0] + '" data-id="' + suffix + '" id="row_' + suffix + '">' +
                ...
                '" name="quantity_amount[]" onfocus="this.select();" class="form-control aligning countID"  placeholder="Cant/Monto" onkeyup="return calculateAll();" ><span class="label_aligning">' +
            ...
            $('#production_cart tbody').append(cart_row);
    ...
    // Validate form
    $(document).on('submit', '#production_form', function() {
        let supplier_id = $("#supplier_id").val();
        let payment_id = $("#payment_id").val();
        let date = $("#date").val();
        //let note = $("#note").val();
        let paid = $("#paid").val();
        let ingredientCount = $("#production_cart tbody tr").length;
        let error = false;
...
function updateRowNo() {
    let numRows = $("#production_cart tbody tr").length;
    for (let r = 0; r < numRows; r++) {
        $("#production_cart tbody tr").eq(r).find("td:first p").text(r + 1);
    }
}
